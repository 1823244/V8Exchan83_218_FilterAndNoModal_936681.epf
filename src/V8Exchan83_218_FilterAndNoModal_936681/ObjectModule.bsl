		...
		Если АлгоритмыОбработчика.Количество() <> 0 Тогда //В этой строке есть вызовы алгоритмов
			...
			Для Каждого Алгоритм Из АлгоритмыОбработчика Цикл
				...
				Если МассивЗапрошенныхЭлементов.Найти(ВРег(Алгоритм)) <> Неопределено Тогда //Рекурсивный вызов алгоритма
					...
					СтрокаОператора = "ВызватьИсключение ""РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА: "+ Алгоритм + """;";
...
Функция ПолучитьВыборкуДляВыгрузкиСОграничениями(Правило, ВыборкаДляПодстановкиВПКО = Неопределено, Свойства = Неопределено)
	...
	Если Правило.НастройкиПостроителя <> Неопределено Тогда
		Если ТипЗнч(Правило.НастройкиПостроителя)= Тип("ОтборКомпоновкиДанных") Тогда
			ЗаполнитьОтборыПостроителяПоОтборамКомпоновки(ПостроительОтчета.Отбор, Правило.НастройкиПостроителя.Элементы, ИмяМетаданных);			
		Иначе
			ПостроительОтчета.УстановитьНастройки(Правило.НастройкиПостроителя);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьОтборПоОтборуКомпоновщика(Отбор, ОтборКомпоновщика)
	
	ЗаполнитьЗначенияСвойств(Отбор, ОтборКомпоновщика, "Использование, Представление");
	
	ЗначениеОтбора = ОтборКомпоновщика.ПравоеЗначение;
	...
	ИначеЕсли ОтборКомпоновщика.ВидСравнения = ВидСравненияКомпоновкиДанных.Заполнено Тогда
		Отбор.ВидСравнения = ВидСравнения.НеРавно;
		ЗначениеОтбора = Отбор.ТипЗначения.ПривестиЗначение(Неопределено);
	ИначеЕсли ОтборКомпоновщика.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено Тогда
		Отбор.ВидСравнения = ВидСравнения.Равно;
		ЗначениеОтбора = Отбор.ТипЗначения.ПривестиЗначение(Неопределено);
	Иначе
	...
	Если ТипЗнч(ЗначениеОтбора) = Тип("СтандартнаяДатаНачала") Тогда
		ЗначениеОтбора = ЗначениеОтбора.Дата;
	КонецЕсли;
...
//Заполнение отборов построителя по отборам компоновщика
Процедура ЗаполнитьОтборыПостроителяПоОтборамКомпоновки(ОтборыПостроителя, ОтборыКомпоновки, ИмяОбъекта) Экспорт
	
	Для каждого ОтборКомпоновки Из ОтборыКомпоновки Цикл
		ОтборПостроитель = ОтборыПостроителя.Добавить(СтрЗаменить(ИмяОбъекта, ".", "_") 
								+ "." + Строка(ОтборКомпоновки.ЛевоеЗначение));
		ЗаполнитьОтборПоОтборуКомпоновщика(ОтборПостроитель, ОтборКомпоновки);
	КонецЦикла;
...
Процедура ИнициализацияВнешнейОбработкиОбработчиковСобытий(РаботаВозможна, ОбъектВладелец) Экспорт
	...
	Попытка // если есть БСП
		ЭтоРазделеннаяКонфигурация = Вычислить("ОбщегоНазначенияПовтИсп.РазделениеВключено()");
	Исключение
	...
	Если ЭтоРазделеннаяКонфигурация Тогда
		...
		Если Не ФлагРежимОтладкиОбработчиков Или ЭтоВнешняяОбработка Тогда
			ВызватьИсключение(Нстр("ru = 'Загрузка данных в модели сервиса может выполняться только в безопасном режиме.'"));
		КонецЕсли;
	КонецЕсли;
